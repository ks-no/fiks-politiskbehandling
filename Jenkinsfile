pipeline {
    agent any
    environment {
        PROJECT_FOLDER = "ks.fiks.io.politiskbehandling.sample"
        PROJECT_CHARTNAME = "fiks-politiskbehandling-simulator"
        APP_NAME = "fiks-politiskbehandling-simulator"
    ...
    stages {
        ...
        stage("fiks-politiskbehandling-simulator: Build and publish docker image") {
            steps {
                dir("Eksempelklienter/ks.fiks.io.politiskbehandling.sample") {
                    println("fiks-politiskbehandling-simulator: Building and publishing docker image version: ${env.FULL_VERSION}")
                    buildAndPushDockerImageFiksArkiv(APP_NAME, "./${PROJECT_FOLDER}/Dockerfile", [env.FULL_VERSION, 'latest'], [], params.isRelease)
                }
            }
        }
...
def findVersionSuffix() {
    ...
    def findCommand = $/find Eksempelklienter/ks.fiks.io.politiskbehandling.sample -name "ks.fiks.io.politiskbehandling.sample.csproj" -exec xpath '{}' '/Project/PropertyGroup/VersionPrefix/text()' \;/$
